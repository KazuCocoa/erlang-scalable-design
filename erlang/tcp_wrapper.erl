...
-export ([init/3, system_continute/3, system_terminate/4, init_request/2]).

-callback init_request() -> {'ok', Reply :: term()}.
-callback get_request(Data :: term(), LoopData :: term()) -> {'ok', Reply :: term()} | {'stop', Reason :: atom(), LoopData :: term()}.
...
cast(Host, Port, Data) ->
  {ok, Socket} = gen_tcp:connect(Host, Port, [binary, {active, fales}, {reuseaddr, true}]),
  send(Socket, Data),
...
system_continute(Parent, Debug, {Listener, Mod}) ->
  loop(Mod, Listener, Parent, Debug).
...
debug(Dev, Event, Data) ->
  io:format(Dev, "Listner ~w:~w~n", [Event, Data]).
...
get_request(Mod, Socket, LoopData) ->
  case gen_tcp:recv(Socket, 0) of
    {ok, Data} ->
      case Mod:gen_request(Data, LoopData) of
        {ok, NewLoopData} ->
